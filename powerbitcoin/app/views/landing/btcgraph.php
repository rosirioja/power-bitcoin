...
<script type="text/javascript">
	...
	function graph(){
		...
		$('.graph-label').html("<center>Graph of Mining Rig's Voltage (in Volts) </center>");
		break;

		case currentdata:
		maxcurrent = <?php echo $maxcurrent; ?>;
		stepwidth = Math.ceil(maxcurrent / 5);
		$('.graph-label').html("<center>Graph of Mining Rig's Current (in MilliAmperes) </center>");
		break;

		case powerdata:
		maxpower = <?php echo $maxpower; ?>;
		stepwidth = Math.ceil(maxpower / 5);
		$('.graph-label').html("<center>Graph of Mining Rig's Power (in Watts) </center>");
		break;

		case tempdata:
		maxtemp = <?php echo $maxtemp; ?>;
		stepwidth = Math.ceil(maxtemp / 5);
		$('.graph-label').html("<center>Graph of Mining Rig's Temperature (in Celsius) </center>");
		break;
		}
